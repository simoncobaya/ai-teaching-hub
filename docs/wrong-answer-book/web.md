...
> 在 `app/wrong-answer-book/` 目录下执行。
> 💡 Windows 用户：路径分隔符用 `\`，如 `python web_app.py --work-dir C:\Users\me\my-wab`。

```bash
python web_app.py
```

> 💡 **不会敲命令？** 把下面这句话复制到 CodeBuddy 对话框，让 AI 帮你启动：
...
python web_app.py --work-dir /home/user/my-wab         # 指定工作目录（Windows: --work-dir C:\Users\me\my-wab）
