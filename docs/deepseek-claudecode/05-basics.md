# 📖 Claude Code 基础用法 + 实用技巧

> DeepSeek 已经配置好了！现在让我们学会怎么和 Claude Code 对话，让它帮你写代码 🎉

---

> ⚠️ **重要提示**：你需要 **先完成 DeepSeek 配置**（下一页的教程），才能实际操作下面的练习！
>
> 配置完成后，再回来学习使用方法。

---

## 🎯 目标

学完这一节，你将知道：
- ✅ 如何启动和退出 Claude Code
- ✅ 如何和 Claude Code 对话
- ✅ 常用的命令
- ✅ 让 AI 更听话的秘诀

---

## 🚀 启动 Claude Code

### Step 1：创建一个项目文件夹

首先，创建一个文件夹来放你的代码：

**macOS / Linux（终端）：**

```bash
mkdir my-first-project
cd my-first-project
```

**Windows（PowerShell 或 CMD）：**

```powershell
mkdir my-first-project
cd my-first-project
```

> 💡 这几个系统的命令其实是一样的！都可以直接复制粘贴。

### Step 2：启动 Claude Code

```bash
claude
```

你会看到 Claude Code 的欢迎界面！🎉

### Step 3：信任项目文件夹（首次使用时）

第一次在一个新文件夹里使用 Claude Code 时，它会问你一个问题：

```
Accessing workspace: /你的项目文件夹路径

Quick safety check: Is this a project you created or one you trust?

❯ 1. Yes, I trust this folder
   2. No, exit
```

- 如果你**自己创建**了这个文件夹 → 选 **1. Yes, I trust this folder**（按回车）
- 如果你**不确定**这个文件夹里有什么 → 选 **2. No, exit**，先看看再说

> 💡 这是 Claude Code 的安全保护，确保它不会在你不知道的文件夹里乱改文件。
> 下次再打开同一个文件夹，就不会再问这个问题了。

> 💡 **快捷用法**：如果你只想让 AI 做一件事，不用进入对话模式，直接写：
> ```bash
> claude "帮我写一个 Hello World 程序"
> ```
> 这样 Claude Code 会直接帮你完成任务，做完就退出！

---

## 💬 和 Claude Code 对话

启动后，你就可以直接 **打字和 Claude Code 对话** 了！

### 试试这些对话

#### 对话 1：让 AI 写代码

```
你：帮我用 Python 写一个打印"Hello World"的程序
```

Claude Code 会帮你创建一个 Python 文件并写入代码。

#### 对话 2：让 AI 解释代码

```
你：请解释一下这个项目里的代码都在做什么
```

Claude Code 会读取你的项目文件，然后用简单的语言解释。

#### 对话 3：让 AI 修改代码

```
你：请把猜数字游戏的范围改成 1-1000
```

Claude Code 会找到相关文件并修改代码。

#### 对话 4：让 AI 运行程序

```
你：运行猜数字游戏
```

Claude Code 会帮你执行程序。

---

## ⌨️ 常用命令

在 Claude Code 里，以 `/` 开头的是特殊命令：

| 命令 | 作用 | 例子 |
|------|------|------|
| `/help` | 查看帮助 | 显示所有可用命令 |
| `/model` | 切换 AI 模型层级 | `/model sonnet` |
| `/login` | 登录账号 | 重新登录或切换账号 |
| `/config` | 打开设置 | 查看/修改配置 |
| `/clear` | 清空对话 | 开始新的对话 |
| `/compact` | 压缩上下文 | 对话太长时使用 |
| `/quit` | 退出 Claude Code | 结束会话 |

### 最常用的命令

```
/model             ← 查看可用的模型列表
/model sonnet      ← 切换到 Sonnet（日常使用）
/model opus        ← 切换到 Opus（最强，适合难题）
/model haiku       ← 切换到 Haiku（最快，适合简单任务）
```

> 💡 `/model` 只能在 opus、sonnet、haiku 之间切换。如果你用 CC Switch 配置了 DeepSeek，这些模型实际调用的就是 DeepSeek。要在不同供应商（DeepSeek ↔ Claude）之间切换，请在 **CC Switch** 中操作。

---

## 📂 Claude Code 如何操作文件

Claude Code 会 **自动** 帮你操作文件，但每次操作前都会 **问你确认**：

| 操作 | 你会看到 | 你要做的 |
|------|---------|---------|
| 创建文件 | 📝 "创建文件 xxx" | 按 `y` 确认 |
| 修改文件 | ✏️ "修改 xxx 第 10 行" | 按 `y` 确认 |
| 运行命令 | ▶️ "运行命令 xxx" | 按 `y` 确认 |

> 💡 **安全提示**：Claude Code 每次操作都会问你，不会偷偷改你的文件。如果你不确定，可以按 `n` 拒绝。

---

## 💡 提示词技巧

和 Claude Code 对话时，怎么说很重要！

### ✅ 好的提示词

| 场景 | 好的提示词 ✅ |
|------|-------------|
| 写代码 | "帮我用 Python 写一个猜数字游戏，范围是 1-100，有 5 次机会" |
| 改代码 | "把第 10 行的颜色从红色改成蓝色" |
| 找 bug | "运行这个程序报错了，错误信息是 TypeError，帮我找问题" |
| 解释 | "用小学生能听懂的话解释这个函数" |

### ❌ 不太好的提示词

| 场景 | 不太好的提示词 ❌ | 为什么不好 |
|------|-----------------|-----------|
| 写代码 | "写个东西" | 太模糊，AI 不知道写什么 |
| 改代码 | "改一下" | 没说改什么 |
| 找 bug | "有 bug" | 没说是什么 bug |

### 💡 好提示词的公式

```
做什么 + 用什么语言 + 具体要求 + 举例说明
```

举个例子：

```
做什么：写一个猜数字游戏
用什么语言：Python
具体要求：范围 1-100，有 5 次机会，猜完告诉答案
```

合在一起就是：

```
帮我用 Python 写一个猜数字游戏：
- 范围是 1-100
- 玩家有 5 次机会
- 猜完要告诉正确答案
```

> 🌱 古人说 **"磨刀不误砍柴工"**。花 10 秒想清楚怎么说，比花 10 分钟改代码划算多了！

---

## ⌨️ 快捷键

| 快捷键 | 作用 |
|--------|------|
| `Enter` | 发送消息 |
| `Shift + Enter` | 换行（不发送） |
| `Ctrl + C` | 取消当前操作 |
| `Ctrl + C` 两次 | 退出 Claude Code |
| `Tab` | 接受建议 |

---

## 🧠 让 AI 记住你

你有没有发现：**每次打开 Claude Code，它都是"空白的"**，完全不记得你上次跟它说过什么？

就像你告诉朋友你的名字，第二天他又问一遍……会很烦 😅

Claude Code 提供了两个"记忆"功能来解决这个问题：

### 📝 CLAUDE.md — 你的 AI 备忘录

在你的项目文件夹里创建一个 `CLAUDE.md` 文件，写上你想让 AI 记住的事情：

```markdown
# 我的猜数字游戏

## 项目说明
这是一个猜数字游戏。

## 技术栈
- HTML + CSS + JavaScript
- 用浏览器打开即可运行

## 规则
- 代码注释用中文
- 变量名用英文
- 测试前先运行 npm test
```

Claude Code 每次启动都会先读这个文件，就像你写了一张小纸条贴在电脑上 📌

> 💡 **写什么好？**
> - 你的项目是做什么的
> - 你喜欢的编程风格（比如"注释用中文"）
> - 常用的命令（比如"每次改完代码先测试"）

### 🧠 自动记忆 — AI 自己记笔记

除了你写的 CLAUDE.md，Claude Code 还有一个**自动记忆**功能：当你纠正 AI 的时候，AI 会自己记下来。

比如你说了 3 次"不要用 `var`，用 `const`"，AI 会自己记到备忘录里，下次就不会忘了。

> 💡 自动记忆默认开启，不需要额外配置。AI 会自动判断哪些事情值得记住。

---

## 🛠️ AI 的"工具箱"

还记得 [Claude Code 的起源故事](./03-claude-code.md) 里说的吗？Boris 给了 AI "手和脚"。

这些"手和脚"的专业名字叫 **Tool Calls（工具调用）**。每次 AI 做这些事情，其实都是在调用一个工具：

| 你看到的 | 背后是 |
|---------|--------|
| 📝 创建文件 | `Write` 工具 |
| ✏️ 修改代码 | `Edit` 工具 |
| ▶️ 运行命令 | `Bash` 工具 |
| 🔍 搜索文件 | `Grep` / `Glob` 工具 |
| 🌐 查资料 | `WebSearch` 工具 |

你已经见过这些"工具"了！从第六步的第一个项目开始，AI 就一直用它们帮你编程。了解这个名字，你就更清楚 AI 是怎么工作的了 👏

---

## 🤖 AI 的小帮手（了解）

Claude Code 还有一个厉害的功能：**Subagents（子代理）**。

简单理解：当你让 AI 做一件大事时，AI 可以**派出几个"小帮手"** 同时去干不同的活。

就像老师布置一个项目，让一些同学查资料、一些同学写代码、一些同学做测试。

> 💡 这个功能比较高级，你现在只需要知道"有这个功能"就行。以后参加比赛或者做大项目的时候，可以试试它！

---

## 🔄 上下文管理

### 对话太长怎么办？

当你和 Claude Code 聊了很久，它可能会"忘记"前面的内容。这时候：

```
/compact
```

这个命令会让 Claude Code **总结**之前的对话，释放空间继续聊。

### 完全重新开始

```
/clear
```

这个命令会 **清空** 所有对话，从头开始。

---

## 🎯 常见场景的提示词模板

### 场景 1：从零开始做项目

```
我想做一个 [项目名称]，用 [编程语言]。

功能要求：
1. [功能 1]
2. [功能 2]
3. [功能 3]

请先帮我设计项目结构，然后逐步实现。
```

### 场景 2：修复 Bug

```
我的程序出了问题：

错误信息是：
[粘贴错误信息]

相关代码在 [文件名] 文件里。

请帮我找到问题并修复。
```

### 场景 3：优化代码

```
请帮我优化 [文件名] 里的代码：
- 让它运行更快
- 让代码更容易阅读
- 加上中文注释
```

### 场景 4：学习理解

```
请用小学生能听懂的话解释 [概念名称]：
1. 它是什么
2. 为什么要用它
3. 用一个生活中的例子来说明
4. 写一段简单的示例代码
```

---

## 🎮 实战练习

让我们用 Claude Code 写一个简单的程序！

### 练习 1：Hello World

1. 启动 Claude Code
2. 输入：
   ```
   帮我用 Python 写一个 Hello World 程序
   ```
3. 按 `y` 确认创建文件
4. 输入：
   ```
   运行这个程序
   ```
5. 按 `y` 确认运行

### 练习 2：让 AI 做网页

试试这些有趣的对话：

```
帮我用 HTML 写一个显示"我喜欢 AI 🤖"的网页，要有好看的背景和字体
```

```
帮我用 HTML + JavaScript 写一个九九乘法表网页
```

---

## 🔄 退出 Claude Code

对话结束后，你可以用以下方式退出：

- 输入 `/quit`
- 或按 `Ctrl + C` 两次

---

## ❓ 常见问题

### Q：Claude Code 需要联网吗？
**是的！** Claude Code 需要联网才能和 AI 模型对话。

### Q：第一次打开 Claude Code 会怎样？

- **已配置 DeepSeek**：直接就能用，不需要登录 Claude 账号
- **未配置 DeepSeek**：Claude Code 会让你登录 Claude 账号（需要 Claude 订阅）。如果你没有订阅，请先去看下一页的配置教程

### Q：Claude Code 会自动保存吗？
**是的！** Claude Code 创建和修改的文件会直接保存到你的项目文件夹里。

### Q：我可以拒绝 Claude Code 的操作吗？
**当然可以！** 每次操作都会问你，按 `n` 就可以拒绝。

---

[⬅️ 上一页：配置 DeepSeek](./04-config.md) | [➡️ 下一页：第一个项目](./06-verify.md)
