...
**解决方法**：
1. 使用更具体的描述（参考 [提示词技巧](./05-basics.md)）
2. 提供更多上下文信息
3. 尝试用 `/model opus` 切换到更强的模型层级

### ❓ 打地鼠游戏报声音设备错误（"Audio device" / "No available audio device"）

**可能原因**：电脑没有声卡或音频设备不可用（比如在远程桌面上运行游戏）。

**解决方法**：
- 这不是代码错误，是电脑环境问题。如果游戏能正常打开、能玩，只是没有声音，**不用管它**。
- 想让游戏在这种电脑上也能安静地运行，让 AI 加一句：把音效初始化用 try/except 包起来，音频设备不可用时自动静音继续运行。

### ❓ 打地鼠游戏报视频设备错误（"No available video device"）

**可能原因**：电脑没有显示器/显卡驱动异常（常见于远程桌面或服务器环境）。

**解决方法**：
1. 确认你是不是在远程桌面里运行——在远程桌面里 pygame 可能无法创建窗口
2. 在本地电脑上运行游戏（不要用远程桌面）
3. 确认显卡驱动已安装更新
...
3. 💬 问老师或同学
4. 🔄 试试用 **CC Switch** 重新配置（参考 [配置教程](./04-config.md)）

---

[⬅️ 上一页：疯狂打地鼠（下）](./08-game-feel.md)
