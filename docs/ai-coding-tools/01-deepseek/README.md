# 🐋 DeepSeek 是什么？

> DeepSeek（深度求索）是一个超级聪明的 AI，特别擅长编程！而且它 **非常便宜**，是我们教程推荐的 AI 模型。

---

## 🤔 DeepSeek 是什么？

DeepSeek 是一家中国 AI 公司（深度求索）开发的 AI 大模型。你可以把它想象成一个 **超级聪明的编程老师**：

- 📝 你用文字告诉它你想做什么
- 💻 它帮你写出完整的代码
- 🐛 你遇到 bug，它帮你找出来并修复
- 💡 你不懂的知识，它用简单的话解释给你听

---

## 🌟 为什么推荐 DeepSeek？

### 1. 💰 超级便宜！

| AI 模型 | 输入价格（每百万字） | 输出价格（每百万字） |
|---------|-------------------|-------------------|
| **DeepSeek V3** 🐋 | **$0.14**（约 ¥1） | **$0.28**（约 ¥2） |
| Claude Sonnet | $3.00（约 ¥22） | $15.00（约 ¥108） |
| GPT-4o | $2.50（约 ¥18） | $10.00（约 ¥72） |

> 🎯 DeepSeek 比 Claude 便宜 **20-50 倍**！对学生来说非常友好。

### 2. 🧠 编程能力很强

DeepSeek 在编程方面的能力 **和顶级 AI 不相上下**：

- 代码生成质量高
- 支持 Python、JavaScript、HTML/CSS 等主流语言
- 能理解整个项目，不只是单个文件

### 3. 🇨🇳 中文支持好

DeepSeek 是中国团队开发的，**中文理解能力非常好**：
- 你可以用中文和它对话
- 它生成的中文注释和文档很自然
- 适合国内学生使用

### 4. 🔌 容易接入

DeepSeek 使用 **OpenAI 兼容格式** 的 API，这意味着：
- 很多 AI 编程工具都可以直接用它
- 配置非常简单
- 切换模型很方便

---

## 📦 DeepSeek 的模型

DeepSeek 提供了几个不同的模型：

| 模型 | 用途 | 特点 |
|------|------|------|
| `deepseek-chat` | 日常编程 | 速度快，适合大部分任务 |
| `deepseek-reasoner` | 复杂推理 | 会"思考"更久，适合难题 |

> 💡 **建议先用 `deepseek-chat`**，遇到特别难的问题再切换到 `deepseek-reasoner`。

---

## 🔗 相关链接

| 名称 | 链接 |
|------|------|
| DeepSeek 官网 | https://platform.deepseek.com/ |
| DeepSeek API 文档 | https://api-docs.deepseek.com/ |
| DeepSeek 聊天界面 | https://chat.deepseek.com/ |

---

## 📌 下一步

了解 DeepSeek 是什么之后，接下来：

👉 **[注册账号 + 获取 API Key](./register.md)** — 你需要一个 API Key 才能使用 DeepSeek

👉 **[在线体验 DeepSeek](./playground.md)** — 先在网页上试试 DeepSeek 的能力
